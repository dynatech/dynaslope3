...
def quick_inbox():
    quick_inbox_query = text("SELECT * FROM "
                             ...
                             "FROM "
                             "(SELECT "
                             "comms_db_3.smsinbox_users.inbox_id,"
                             "comms_db_3.smsinbox_users.ts_sms,"
                             "comms_db_3.smsinbox_users.mobile_id,"
                             "comms_db_3.smsinbox_users.sms_msg,"
                             "comms_db_3.smsinbox_users.read_status,"
                             "comms_db_3.smsinbox_users.web_status,"
                             "comms_db_3.smsinbox_users.gsm_id,"
                             "comms_db_3.user_mobile.sim_num,"
                             ...
                             "FROM "
                             "comms_db_3.smsinbox_users "
                             "INNER JOIN comms_db_3.user_mobile ON comms_db_3.smsinbox_users.mobile_id = comms_db_3.user_mobile.mobile_id "
                             "INNER JOIN commons_db.users ON comms_db_3.user_mobile.user_id = commons_db.users.user_id "
                             ...
                             "WHERE "
                             "comms_db_3.smsinbox_users.ts_sms > (NOW() - INTERVAL 70 DAY)) AS smsinbox "
                             ...
                             "FROM "
                             "comms_db_3.smsinbox_users "
                             "INNER JOIN comms_db_3.user_mobile ON smsinbox_users.mobile_id = comms_db_3.user_mobile.mobile_id "
                             "INNER JOIN commons_db.users ON comms_db_3.user_mobile.user_id = commons_db.users.user_id "
                             "INNER JOIN commons_db.user_organization ON commons_db.users.user_id = commons_db.user_organization.user_id "
                             "INNER JOIN sites ON commons_db.user_organization.fk_site_id = sites.site_id "
                             "WHERE "
                             "comms_db_3.smsinbox_users.ts_sms > (NOW() - INTERVAL 70 DAY) "
                             "ORDER BY comms_db_3.smsinbox_users.ts_sms DESC) AS smsinbox2 USING (inbox_id) "
                             ...
                             "comms_db_3.smsinbox_users.gsm_id,"
                             "comms_db_3.user_mobile.sim_num,"
                             "CONCAT(comms_db_3.user_teams.team_code, ' - ', commons_db.users.last_name, ', ', commons_db.users.first_name) AS full_name "
                             "FROM "
                             "comms_db_3.smsinbox_users "
                             "INNER JOIN comms_db_3.user_mobile ON comms_db_3.smsinbox_users.mobile_id = comms_db_3.user_mobile.mobile_id "
                             "INNER JOIN commons_db.users ON comms_db_3.user_mobile.user_id = commons_db.users.user_id "
                             "INNER JOIN commons_db.user_team_members ON commons_db.users.user_id = commons_db.user_team_members.users_users_id "
                             "INNER JOIN comms_db_3.user_teams ON commons_db.user_team_members.user_teams_team_id = comms_db_3.user_teams.team_id "
                             "WHERE comms_db_3.smsinbox_users.ts_sms > (NOW() - INTERVAL 70 DAY)) AS smsinbox GROUP BY full_name) AS quickinbox "
                             "INNER JOIN (SELECT "
                             "comms_db_3.smsinbox_users.inbox_id,"
                             "comms_db_3.smsinbox_users.ts_sms,"
                             "comms_db_3.smsinbox_users.mobile_id,"
...
def unregistered_inbox():
    ...
    unregistered_inbox_query = text("SELECT "
                                    ...
                                    "comms_db_3.user_mobile.sim_num,"
                                    "comms_db_3.user_mobile.mobile_id,"
                                    "comms_db_3.user_mobile.user_id,"
                                    "comms_db_3.smsinbox_users.sms_msg,"
                                    "comms_db_3.smsinbox_users.ts_sms "
                                    "FROM "
                                    "comms_db_3.smsinbox_users "
                                    "INNER JOIN "
                                    "comms_db_3.user_mobile ON comms_db_3.smsinbox_users.mobile_id = comms_db_3.user_mobile.mobile_id "
                                    "INNER JOIN "
                                    "commons_db.users ON comms_db_3.user_mobile.user_id = commons_db.users.user_id "
                                    "WHERE comms_db_3.smsinbox_users.ts_sms > (NOW() - INTERVAL 70 DAY) "
...
def get_conversation():
    ...
    smsinbox_query = text("SELECT comms_db_3.smsinbox_users.inbox_id as convo_id, comms_db_3.smsinbox_users.mobile_id,"
                          "comms_db_3.smsinbox_users.ts_sms as ts_received,null as ts_written,null as ts_sent,comms_db_3.smsinbox_users.sms_msg,"
                          "comms_db_3.smsinbox_users.web_status,comms_db_3.smsinbox_users.gsm_id,"
                          "null as send_status , comms_db_3.smsinbox_users.ts_sms as timestamp,'" +
                          details["full_name"] +
                          "' as user from comms_db_3.smsinbox_users "
                          "WHERE " + str(conversation_details["query_builder"]["inbox_query_builder"]) + "")

    smsoutbox_query = text("SELECT comms_db_3.smsoutbox_users.outbox_id as convo_id, comms_db_3.smsoutbox_user_status.mobile_id,"
                           "null as ts_received,comms_db_3.smsoutbox_users.ts_written,comms_db_3.smsoutbox_user_status.ts_sent,comms_db_3.smsoutbox_users.sms_msg,"
                           "comms_db_3.smsoutbox_user_status.web_status,comms_db_3.smsoutbox_user_status.gsm_id,comms_db_3.smsoutbox_user_status.send_status,"
                           "comms_db_3.smsoutbox_users.ts_written as timestamp,"
                           "'You' as user FROM comms_db_3.smsoutbox_users "
                           "INNER JOIN comms_db_3.smsoutbox_user_status ON smsoutbox_users.outbox_id = comms_db_3.smsoutbox_user_status.outbox_id "
...
def get_user_mobile_details(details):
    ...
    query = text("SELECT "
                 ...
                 "comms_db_3.user_mobile.mobile_id "
                 "FROM "
                 "comms_db_3.user_mobile "
                 "JOIN "
                 "commons_db.users ON commons_db.users.user_id = comms_db_3.user_mobile.user_id "
                 "WHERE "
...
def mobile_query_builder(recipients_mobile_id):
    ...
    for mobile_id in recipients_mobile_id:
        if(counter == 0):
            inbox_query = "comms_db_3.smsinbox_users.mobile_id = " + \
                str(mobile_id) + ""
            outbox_query = "comms_db_3.smsoutbox_user_status.mobile_id = " + \
                str(mobile_id) + ""
        else:
            inbox_query = inbox_query + \
                " OR comms_db_3.smsinbox_users.mobile_id = " + \
                str(mobile_id) + ""
            outbox_query = outbox_query + \
                " OR comms_db_3.smsoutbox_user_status.mobile_id = " + \
                str(mobile_id) + ""
