...
class GsmServer:
	...
	def send_messages_from_db(self, gsm = None, table = 'users', send_status = 5 , 
		...
		for msg in allmsgs:
			table_mobile = db.getAllUsersMobile(msg[1], mobile_id_flag = True) # Refactor this one
		...
		if len(error_stat_list) > 0:
			print(">> Ignoring invalid messages...", smstables.set_send_status(table, error_stat_list))
			print("done")
