...
class GsmModem:
	...
	REPLY_TIMEOUT = 30 # SET CONSTANT
	SEND_INITIATE_REPLY_TIMEOUT = 20 # SET CONSTANT
	SENDING_REPLY_TIMEOUT = 60 # SET CONSTANT
	RESET_DEASSERT_DELAY = 1 # SET CONSTANT
	RESET_ASSERT_DELAY = 5     # delay when module power is off
	WAIT_FOR_BYTES_DELAY = 0.5 # SET CONSTANT
	POWER_ON_DELAY = 3 # SET CONSTANT
	...
	def setGSMDefaults(self):
		...
		try:
			...
			print(self.executeATCmd('AT+CMGF=0').rstrip('\r\n'))
			print("Disabling unsolicited CMTI",)
			print(self.executeATCmd('AT+CNMI=2,0,0,0,0').rstrip('\r\n'))
			return True
	...
	def sendSMS(self, msg, number):
		...
		temp_pdu = '0001000C813'+str(pdulist[0])[11:] # SET CONSTANT
		pdulist[0] = temp_pdu
