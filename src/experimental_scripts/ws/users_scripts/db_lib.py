...
class DatabaseConnection:
    ...
    def write_inbox(self, msglist='', gsm_info=''):
        ...
        for msg in msglist:
            ...
            query_loggers = self.write_logger_sms(
                msg, gsm_id, ts_sms, ts_stored, sms_msg)
            if (query_loggers == -1):
                query_users = self.write_users_sms(
                    msg, gsm_id, ts_sms, ts_stored, sms_msg)
    ...
    def get_gsm_info(self, gsm_id):
        gsm_dict = {}
        query = "SELECT * FROM mia_comms_db_3.gsm_modules where gsm_id = '" + \
                str(gsm_id) + "';"
